...
public:
  ...
  inline Vector4D operator-() const { return Vector4D(-x, -y, -z, -a); }
  ...
  // Vector Length
  GLfloat Length() const { return sqrtf(x * x + y * y + z * z + a * a); }

  GLfloat Distance(const Vector4D &V1) const { return (*this - V1).Length(); }
